...
class StopElementPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: StreamBuilder(
            stream: Stream.periodic(const Duration(milliseconds: 1000),(x) => getStop(idParada).asStream()).asyncExpand((x) => x) ,
            builder: (bctx, data) {
              final parada = data.data as IndividualStop;
              return parada?.linesTime?.length == null || parada?.linesTime?.length < 1
                  ? Align(
  ...
  Widget build(BuildContext ctx) {
    return Card(
      child: Column(
        children: <Widget>[
          Align(
            child: Text(parada?.sinoptico, textScaleFactor: 4,style: TextStyle(color: Colors.white)),
            alignment: Alignment.center,
          ),
          Text(
            parada?.nombre ?? "Sen Informacion",textScaleFactor: 1,
            style: TextStyle(color: Colors.white),
          ),
          Text("${parada.minutosProximoPaso} min",textScaleFactor: 3,style: TextStyle(color: Colors.white))
        ],
      ),
